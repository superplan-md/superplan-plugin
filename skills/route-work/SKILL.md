...
- `using-superplan` has decided Superplan should at least evaluate engagement
...
Route to `context-bootstrap-sync` first when:
...
- `shape-work` for `direct`, `task`, `slice`, or `program`
- `context-bootstrap-sync` when context is the real blocker
