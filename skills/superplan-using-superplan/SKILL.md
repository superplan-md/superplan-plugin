---
name: superplan-using-superplan
...
2. `superplan-using-superplan` entry discipline
...
- route to `superplan-route-work`
- route to `superplan-context-bootstrap-sync` when missing context is the real blocker
- route to `superplan-execute-task-graph` when tracked work is already shaped and should move forward
- route to `superplan-review-task-against-ac` when the real request is completion authority
...
4. execute through the workflow spine, especially `superplan-execute-task-graph`, instead of ad hoc task mutation
...
7. if the request is new or the structure decision is still open, route to `superplan-route-work`

Do not bounce already shaped work back through `superplan-route-work` just because the current message is short.
...
- ambiguity in work definition routes toward the workflow path that will later invoke `superplan-brainstorming`
- execution trouble routes toward the workflow path that may invoke `superplan-systematic-debugging`
- completion claims route toward the workflow path that may invoke `superplan-verification-before-completion`
...
- `superplan-execute-task-graph` for "continue", "pick the next task", "resume T-003", or other already-shaped execution work
- `superplan-review-task-against-ac` for "is this task actually done?", "can this unblock?", or evidence-review requests
- `superplan-context-bootstrap-sync` when serious brownfield work is blocked by missing or stale durable context
- `superplan-route-work` when the engagement or depth choice is still unresolved
...
- sending already shaped work back to `superplan-route-work` by reflex
...
- If the repo is initialized but serious brownfield context is missing or stale, route to `superplan-context-bootstrap-sync`.
- If the request targets existing tracked work, resume the owning later phase instead of forcing a fresh routing pass.
- If the request is repo work but the structure decision is still open, route to `superplan-route-work`.
...
- route to `superplan-context-bootstrap-sync`
- route to `superplan-route-work`
- route to `superplan-execute-task-graph`
- route to `superplan-review-task-against-ac`
