...
`superplan-using-superplan` is not just a trigger router.
...
- "This sounds ambiguous, so go straight to `superplan-brainstorming`."
- "This is a bug, so go straight to `superplan-systematic-debugging`."
...
- "It is probably superplan-route-work."
...
Do not force those requests back through `superplan-route-work` unless the structure itself is now in doubt.
