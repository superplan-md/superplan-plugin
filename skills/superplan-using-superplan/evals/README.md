...
- confirm it routes to `superplan-route-work` rather than choosing `superplan-brainstorming` or `superplan-systematic-debugging` as the entry lane
- confirm "continue T-003" routes to `superplan-execute-task-graph`, not `superplan-route-work`
- confirm "is this task done?" routes to `superplan-review-task-against-ac`, not `superplan-execute-task-graph`

## Handoff Check

- if setup or init is missing, the output is readiness guidance rather than a workflow handoff
- if context is the missing readiness layer, handoff is `superplan-context-bootstrap-sync`
- if engagement is warranted but depth is unknown, handoff is `superplan-route-work`
- if work is already shaped and should continue, handoff is `superplan-execute-task-graph`
- if the request is completion authority, handoff is `superplan-review-task-against-ac`
...
- init present but serious brownfield context missing: route `superplan-context-bootstrap-sync`
