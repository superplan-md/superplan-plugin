---
name: superplan-execute-task-graph
...
- structural: pause affected work and route to `superplan-shape-work`
- strategic: stop broad execution and route upward to `superplan-route-work`
...
8. if overlay support is enabled for the workspace, expect `superplan run`, `superplan run <task_id>`, and `superplan task reopen` to auto-reveal the overlay as work becomes active; on a fresh machine or after install/update, verify overlay health with `superplan doctor --json` and `superplan overlay ensure --json` before assuming it is working, and inspect launchability or companion errors if the reveal fails; use `superplan overlay hide --json` when the workspace becomes idle or empty
...
- back to `superplan-shape-work` when structural trajectory change is detected
- back to `superplan-route-work` only when strategic trajectory change is detected
...
Execution handoff to `superplan-review-task-against-ac` should name the evidence gathered and the CLI/runtime facts that support the review.
