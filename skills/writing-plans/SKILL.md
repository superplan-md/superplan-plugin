...
- expected evidence
- dependency order
- next handoff to `shape-work` or `execute-task-graph`
...
Normal handoff is to `shape-work` or `execute-task-graph`, depending on whether artifact shaping is still needed or the execution frontier is already clear.

If the plan still needs durable task contracts or graph structure, hand off to `shape-work`.
If the frontier is already explicit and bounded, hand off to `execute-task-graph`.
