...
Use these scenarios to test whether `superplan-brainstorming` restores the stronger Superpowers-style ambiguity discipline without absorbing Superplan workflow responsibilities.
...
- never acts as the first lane for repo work that still needs `superplan-using-superplan` or `superplan-route-work`
...
- hands control back to the owning workflow phase, usually `superplan-shape-work`
- stays inside superplan-brainstorming boundaries rather than absorbing `superplan-shape-work`

## Failure Signs

- treats casual ideation as mandatory Superplan work
- becomes the first lane for ambiguous repo work that should have entered through `superplan-using-superplan`
...
- authors graph depth, autonomy policy, or execution sequencing that belongs to `superplan-shape-work` or `superplan-writing-plans`
