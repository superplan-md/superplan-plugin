---
name: superplan-review-task-against-ac
...
4. If no credible proof path exists, do not bless completion.
   Route to:
   - `superplan-verification-before-completion` when proof can still be gathered
...
- to `superplan-verification-before-completion` when key proof is missing, weak, or stale but the task is still reviewable
- back to `superplan-execute-task-graph` for more work when AC are unmet after honest review
...
- back to `superplan-shape-work` when the task contract no longer matches the real work
